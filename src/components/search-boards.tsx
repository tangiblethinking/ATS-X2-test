...
export function BoardsPanel({
  ...
  return (
    <section className="rounded-xl border border-border bg-card">
      ...
      >
        <span>
          <span className="block text-sm font-medium text-foreground">Boards</span>
          <span className="block text-sm text-muted-foreground">
            Add a company from its careers URL. Built-in boards can be turned off, not deleted.
          </span>
        </span>
        <ChevronDown
          className={`size-4 shrink-0 text-muted-foreground motion-safe:transition-transform motion-safe:duration-200 ${open ? "rotate-180" : ""}`}
          aria-hidden="true"
        />
      </button>
      {open && boards ? (
          ...
          <form onSubmit={onAdd} className="flex flex-col gap-2">
            <div className="flex flex-col gap-2 sm:flex-row">
              <input
                ...
                className="min-h-11 w-full rounded-sm border border-border bg-background px-3 text-sm sm:flex-1"
              />
              <select
                value={addSource}
                onChange={(event) => {
                ...
                className="min-h-11 rounded-sm border border-border bg-background px-3 text-sm sm:w-40"
              >
              ...
              <button
                type="submit"
                className="inline-flex min-h-11 items-center justify-center gap-2 rounded-sm border border-border px-4 text-sm font-medium text-foreground"
              >
            ...
            {addError ? <p className="text-sm text-foreground">{addError}</p> : null}
          </form>
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground tabular-nums">{activeCount} included in the next search</p>
            <button type="button" onClick={onRestore} className="inline-flex min-h-11 items-center gap-2 text-sm font-medium text-primary">
              <RotateCcw className="size-4" aria-hidden="true" />
              Restore defaults
            </button>
          </div>
          <ul className="max-h-80 overflow-auto rounded-sm border border-border">
            {listed.map((board) => {
              ...
              return (
                <li key={key} className="border-b border-border last:border-b-0">
                  <div className="flex min-h-11 items-center gap-3 px-3">
                    ...
                    {board.builtin ? null : (
                      <button type="button" onClick={() => onRemove(key)} className="min-h-11 shrink-0 px-2 text-sm text-muted-foreground">
                        Remove
                      </button>
                    )}
                  </div>
